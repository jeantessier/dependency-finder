...
"%JAVA_HOME%\bin\java" %DEPENDENCYFINDER_OPTS% -Dorg.xml.sax.driver=org.apache.xerces.parsers.SAXParser -DDEPENDENCYFINDER_TESTS_VALIDATE=%DF_VALIDATE% -classpath %DEPENDENCYFINDER_HOME%\classes;%DEPENDENCYFINDER_HOME%\src;%DEPENDENCYFINDER_HOME%\lib\log4j.jar;%DEPENDENCYFINDER_HOME%\lib\jakarta-oro.jar;%DEPENDENCYFINDER_HOME%\lib\xml-apis.jar;%DEPENDENCYFINDER_HOME%\lib\xercesImpl.jar;%DEPENDENCYFINDER_HOME%\lib\xalan.jar;%JUNIT_HOME%\junit.jar junit.textui.TestRunner %DEPENDENCYFINDER_CMD_LINE_ARGS%
